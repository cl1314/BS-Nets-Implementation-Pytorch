...
from Utils import zeroPadding, normalization, doPCA, modelStatsRecord, averageAccuracy, ssrn_SS_IN, BandSelect

BANDLIST =  [ 68, 182 , 73 ,111 , 63, 149, 106 ,116 , 58, 144, 154 , 78 ,159, 101, 139,  96,  53,  45,
 177, 164, 121,  83 , 91, 134, 172, 126,  50 , 48 , 88 ,129]
...
img_channels = 30  #For Band Selection purposes
...
for index_iter in range(ITER):
    ...
    best_weights_RES_path_ss4 = 'models/Indian_30_' + str(
        index_iter + 1) + '.hdf5'
...
#    train_indices, test_indices = sampleFixNum.samplingFixedNum(TRAIN_NUM, gt)
    ...
    tic6 = time.clock()
    #print(x_train.reshape(x_train.shape[0], x_train.shape[1], x_train.shape[2], x_train.shape[3], 1).shape)
    #print(y_train.shape)
    #print(type(x_train))
    history_res4_SS_BN = model_res4_SS_BN.fit(
        BandSelect.transform(x_train.reshape(x_train.shape[0], x_train.shape[1], x_train.shape[2], x_train.shape[3], 1),BANDLIST), y_train,
        validation_data=(BandSelect.transform(x_val.reshape(x_val.shape[0], x_val.shape[1], x_val.shape[2], x_val.shape[3], 1),BANDLIST), y_val),
    ...
    loss_and_metrics_res4_SS_BN = model_res4_SS_BN.evaluate(
        BandSelect.transform(x_test.reshape(x_test.shape[0], x_test.shape[1], x_test.shape[2], x_test.shape[3], 1),BANDLIST), y_test,
        batch_size=batch_size)
    toc7 = time.clock()
    ...
    pred_test_res4 = model_res4_SS_BN.predict(
        BandSelect.transform(x_test.reshape(x_test.shape[0], x_test.shape[1], x_test.shape[2], x_test.shape[3], 1),BANDLIST)).argmax(axis=1)
...
modelStatsRecord.outputStats(KAPPA_RES_SS4, OA_RES_SS4, AA_RES_SS4, ELEMENT_ACC_RES_SS4,
                             ...
                             'records/IN_sd-30.txt',
                             'records/IN_sd_element-30.txt')
