    ...
    for i in range(height):
        for j in range(width):
            ...
            if target == 0 :
                ...
                outputs[i][j] = map[i][j]+1
    # ax.imshow(outputs)
    # fig.savefig(savePath, dpi = dpi)
    spectral.save_rgb(savePath, outputs.astype(int), colors=spectral.spy_colors)
...
#    train_indices, test_indices = sampleFixNum.samplingFixedNum(TRAIN_NUM, gt)
    ...
    model_res4_ss.summary()

    pred_test_conv1 = model_res4_ss.predict(BandSelect.transform(all_data.reshape(all_data.shape[0],all_data.shape[1],all_data.shape[2],all_data.shape[3],1),BANDLIST)).argmax(axis=1)
    ...
    # classification_map(y_re, gt_IN, 24, "Cmaps/RES4_SS_IN_.png")
    classification_map(pred_test_conv1.reshape((145, 145)), gt_IN, 24, "Cmaps/Sayantan-30.png")
